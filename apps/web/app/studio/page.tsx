...
const CANVAS_ELEMENTS = [
  { id: 'form', icon: 'clipboard-list', label: 'Form Block' },
  { id: 'workflow', icon: 'zap', label: 'Workflow Step' },
  { id: 'table', icon: 'bar-chart-3', label: 'Data Table' },
  { id: 'agent', icon: 'bot', label: 'Agent Node' },
      ...
      {!spec ? (
        ...
        <>
          <div className={styles.toolbar}>
            <Button onClick={handleGenerate}><ModuleIcon name="sparkles" size={16} /> Generate Solution</Button>
            <ProGate requiredTier="pro" currentTier={user?.plan ?? 'free'} featureName="One-click deploy">
              <Button variant="secondary" onClick={handleDeploy} disabled={!solution}>
                <ModuleIcon name="rocket" size={16} /> Deploy
              </Button>
            </ProGate>
            ...
            <aside className={styles.sidebar}>
              <Card variant="glass" padding="sm">
                ...
                {CANVAS_ELEMENTS.map((el) => (
                  ...
                  >
                    <span><ModuleIcon name={el.icon} size={16} /></span> {el.label}
                  </button>
                ))}
              </Card>
              {solution && (
            ...
            <section className={styles.canvas}>
              <div className={styles.tabs}>
                {(['workflow', 'web', 'mobile'] as const).map((tab) => (
                  ...
                  >
                    <><ModuleIcon name={tab === 'workflow' ? 'zap' : tab === 'web' ? 'globe' : 'smartphone'} size={14} /> {tab === 'workflow' ? 'Workflow' : tab === 'web' ? 'Web' : 'Mobile'}</>
                  </button>
                ))}
              </div>
                    ...
                    {preview.workflowGraph.map((step, i) => (
                      <div key={step.stepId} className={styles.workflowRow}>
                        ...
                        >
                          ...
                          {step.agent && (
                            <Badge variant="pro"><ModuleIcon name="bot" size={14} /> {step.agent}</Badge>
                          )}
                        </button>
