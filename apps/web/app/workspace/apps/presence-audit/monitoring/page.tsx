          ...
          {DEMO_ALERTS.map((alert) => (
            <div key={alert.id} className={styles.listItem}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--ai-text-muted)' }}>via {alert.channel}</span>
              </div>
              <p style={{ fontSize: 13, margin: 0, color: 'var(--ai-text-muted)' }}>{alert.message}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className={styles.list} style={{ marginTop: 12 }}>
          {DEMO_MONITORING_EVENTS.map((evt) => (
            <div key={evt.id} className={styles.listItem}>
              ...
              <p style={{ fontSize: 13, margin: '4px 0 0', color: 'var(--ai-text-muted)' }}>{evt.description}</p>
            </div>
          ))}
        </div>
      </section>
