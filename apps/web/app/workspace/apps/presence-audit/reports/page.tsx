          ...
          <section key={report.id} className={styles.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: 'var(--ai-text-muted)', margin: 0 }}>
            ...
            <p style={{ fontSize: 13, marginTop: 12, color: 'var(--ai-text-muted)' }}>{report.summary}</p>
            {report.sections.map((s) => (
              <div key={s.id} style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--ai-border)' }}>
                <strong style={{ fontSize: 13 }}>{s.title}</strong>
                <p style={{ fontSize: 13, margin: '4px 0 0', color: 'var(--ai-text-muted)' }}>{s.content}</p>
              </div>
            ))}
          </section>
        ))}
