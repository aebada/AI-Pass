...
function getDashboard() {
  ...
  return defaultPresenceAuditPlatform.getDashboard(DEMO_COMPANY.tenantId);
}
...
export default function PresenceAuditDashboardPage() {
  ...
  if (!dashboard) {
    return (
      <PresenceAuditShell>
        <p className={styles.cardTitle}>Demo dashboard unavailable — refresh or check API connection.</p>
      </PresenceAuditShell>
    );
  }
    ...
    <PresenceAuditShell>
      <div className={styles.grid}>
        ...
        <div className={styles.card}>
          ...
          <p className={styles.statValue}>{trustScore ?? '—'}{trustCertified ? <> <ModuleIcon name="check" size={14} /></> : ''}</p>
        </div>
      </div>
      ...
      <div className={styles.twoCol}>
        <section className={styles.card}>
          ...
          {criticalIssues.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>No critical issues</p>
          ) : (
            <div className={styles.list}>
