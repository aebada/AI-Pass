...
import { resolveModel as hubResolveModel } from './model-hub-bridge.js';
import type {
  AuthConfig,
  HubChatRequest,
  HubExecuteRequest,
...
export class ProviderHub {
  ...
  async *streamChat(
    ...
    const preferredId = request.modelId ?? context.preferredModelId;

    // Model Hub resolution — prefer hub-routed model when app context is set (skip for Auto modes)
    if (context.module && preferredId && !preferredId.startsWith('auto-')) {
      const hubRoute = hubResolveModel({
        appId: context.module,
        taskType: context.taskType,
        preferredModelId: preferredId,
        membershipTier: context.membershipTier,
      });
      const hubModel = this.catalog.get(hubRoute.hubModelId);
      if (hubModel) {
        Object.assign(decision, {
          model: hubModel,
          reason: hubRoute.reason,
          fallbackModelIds: hubRoute.fallbackChain,
        });
      }
    }

    if (preferredId?.startsWith('auto-')) {
      const complexity = preferredId.replace('auto-', '') as 'fast' | 'standard' | 'complex';
      const autoTier = complexity === 'complex' ? 'premium' : 'standard';
      if (!this.membership.canAccessModel(context.membershipTier, preferredId, autoTier)) {
        yield {
          type: 'error',
          error: `Auto · ${complexity[0]!.toUpperCase()}${complexity.slice(1)} requires a higher membership tier.`,
        };
        return;
      }
    } else if (
      !this.membership.canAccessModel(
    ...
    const chain = [decision.model.id, ...decision.fallbackModelIds];
    let lastError = 'All providers failed';

    for (const candidateId of chain) {
      const candidate = this.catalog.get(candidateId);
      if (!candidate) continue;
      const health = this.health.check(candidate.providerId);
      if (health.status === 'down') {
        continue;
      }

      const config = this.resolveModelConfig(candidate);
      if (!config.apiKey) {
        lastError = `Provider "${candidate.providerName}" is not configured. Add API keys to .env.local.`;
        continue;
      }
      ...
      try {
        ...
        for await (const chunk of provider.chat(
          ...
          if (chunk.type === 'error') {
            sawError = true;
            lastError = chunk.error ?? 'Provider error';
            break;
          }
        ...
        const credits = this.estimateCredits(candidate, outputChars);
